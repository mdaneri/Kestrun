...
function Update-MarkdownHelpModule {
    ...
    process {
        ...
        foreach ($modulePath in $Path) {
            ...
            $affectedFiles = Update-MarkdownHelp -Session $Session -Path $modulePath -LogPath $LogPath -LogAppend -Encoding $Encoding `
                -AlphabeticParamsOrder:$AlphabeticParamsOrder -UseFullTypeName:$UseFullTypeName -UpdateInputOutput:$UpdateInputOutput -Force:$Force -ExcludeDontShow:$ExcludeDontShow
            $affectedFiles # yeild
        ...
        foreach ($MamlFilePath in $Path) {
            ...
            try {
                ...
                $xml.helpItems.command.relatedLinks | ForEach-Object {
                    if ($_) {
                        $_.InnerXml = '<maml:navigationLink xmlns:maml="http://schemas.microsoft.com/maml/2004/10"><maml:linkText>PLATYPS_DUMMY_LINK' +
                        '</maml:linkText><maml:uri>https://github.com/PowerShell/platyPS/issues/144</maml:uri></maml:navigationLink>' +
                        $_.InnerXml
                    }
                }
...
function MyGetCommand {
    ...
    function getParams([int]$num) {
        ...
        $parameterType = expand3 -property1 'ParameterSets' -num $num -property2 'Parameters' -property3 'ParameterType'
        # this call we need to fill-up ParameterSets.Parameters with metadata
        $parameters = expand2 -property1 'ParameterSets' -num $num -property2 'Parameters'
...
function ConvertPsObjectsToMamlModel {
    ...
    function FillUpSyntaxFromCommand {
        foreach ($ParameterSet in $Command.ParameterSets) {
            ...
            foreach ($Parameter in $ParameterSet.Parameters) {
                ...
                if ($Parameter.Name -eq 'ProgressAction') {
                    continue
                }
    ...
    foreach ($Example in $Help.examples.example) {
        ...
        $MamlExampleObject.Code = @(
            New-Object -TypeName Markdown.MAML.Model.MAML.MamlCodeBlock ($Example.code, 'powershell')
        )
    ...
    $Help.inputTypes.inputType | ForEach-Object {
        ...
        foreach ($in in $Inputs) { $MamlCommandObject.Inputs.Add($in) }
    ...
    Register-ArgumentCompleter -CommandName New-MarkdownHelp -ParameterName Module -ScriptBlock $Function:ModuleNameCompleter `
        -Description 'This argument completer handles the -Module parameter of the New-MarkdownHelp Command.'
