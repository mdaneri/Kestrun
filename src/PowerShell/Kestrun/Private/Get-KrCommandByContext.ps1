...
# -------- Functions only --------
function Get-KrFunctionsByContext {
  ...
  $funcs = if ($IncludeNonExported) {
    Get-Command -Module $Module -All -CommandType Function
  } else {
    Get-Command -Module $Module      -CommandType Function
  }

  $target = 0
  if ($PSCmdlet.ParameterSetName -eq 'AnyOf') {
    foreach ($n in $AnyOf) { $target = $target -bor (_KrNameToMask $n) }
  } else {
    foreach ($n in $AllOf) { $target = $target -bor (_KrNameToMask $n) }
  }
  ...
  foreach ($f in $funcs) {
    $m = Get-KrFunctionContextMask -Function $f
    if ($notMask -ne 0 -and ($m -band $notMask) -ne 0) { continue }  # exclude forbidden bits

    if ($PSCmdlet.ParameterSetName -eq 'AnyOf') {
      if ($Exact) { if ($m -eq $target) { $f } }
      else        { if ( ($m -band $target) -ne 0 ) { $f } }
    } else {
      if ($Exact) { if ($m -eq $target) { $f } }
      else        { if ( ($m -band $target) -eq $target ) { $f } }
    }
  }
}
...
function Get-KrCommandsByContext {
  ...
  $cmds = if ($IncludeNonExported) { Get-Command -Module $Module -All } else { Get-Command -Module $Module }
...
function Get-KrDocSet {
  ...
  #>
    ...
    switch ($Name) {
        'DefinitionOnly' {
            Get-KrCommandsByContext -AnyOf Definition -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'RouteOnly' {
            Get-KrCommandsByContext -AnyOf Route -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'ScheduleOnly' {
            Get-KrCommandsByContext -AnyOf Schedule -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'RouteAndSchedule' {
            Get-KrCommandsByContext -AllOf Route,Schedule -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'ScheduleAndDefinition' {
            Get-KrCommandsByContext -AnyOf ScheduleAndDefinition -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'Runtime' {
            Get-KrCommandsByContext -AnyOf Runtime -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'Everywhere' {
            Get-KrCommandsByContext -AnyOf Everywhere -Exact -Module $Module -IncludeNonExported:$IncludeNonExported
        }
        'ConfigOnly' {
