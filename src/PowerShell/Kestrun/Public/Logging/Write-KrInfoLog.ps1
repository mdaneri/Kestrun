# For backwards compatibility and also shortand for Write-KrInformationLog

function Write-KrInfoLog {
	<#
	.SYNOPSIS
	...
	.EXAMPLE
		PS> Write-KrInfoLog 'Info log message'
	.EXAMPLE
		PS> Write-KrInfoLog -MessageTemplate 'Processed {@Position} in {Elapsed:000} ms.' -PropertyValues $position, $elapsedMs
	.EXAMPLE
		PS> Write-KrInfoLog 'Error occured' -Exception ([System.Exception]::new('Some exception'))
	#>
	...
	Write-KrInformationLog -MessageTemplate $MessageTemplate -Logger $Logger -Exception $Exception -ErrorRecord $ErrorRecord -PropertyValues $PropertyValues -PassThru:$PassThru
}